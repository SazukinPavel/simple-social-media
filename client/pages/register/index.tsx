...
const Register: NextPage = () => {
  const { formState, reset, handleSubmit, register } = useForm<RegisterDto>({
    mode: "onChange",
  });
  const [regLoading, setRegLoading] = useLoading();
  const dispatch = useTypedDispatch();
  const { isAuth, isError, errorMessage } = useTypedSelector(
    (state) => state.auth
  );
      ...
      <div className={[styles.Register].join(" ")}>
        ...
        <form onSubmit={handleSubmit(registerClick)}>
          <FormInput
            registerFunc={() =>
              register("username", {
                ...
                maxLength: {
                  value: 25,
                  message: "Maximum username length 25 characters",
                },
              })
            }
            text={"Username:"}
          ...
          <div className={styles.buttons}>
            ...
            <LoadingButton isLoading={regLoading} type="submit">
              Register
            </LoadingButton>
          </div>
        </form>
