...
const EditPostModal: React.FC<EditPostModalProps> = ({ post, ...props }) => {
  ...
  const { register, formState, handleSubmit } = useForm<UpdatePostDto>({
    mode: "onSubmit",
  });
    ...
    >
      ...
      <form className={styles.Form} onSubmit={handleSubmit(updatePost)}>
        <FormInput
          registerFunc={() =>
            register("text", {
              ...
              maxLength: {
                value: 25,
                message: "Maximum length 215 characters",
              },
            })
          }
          text={"Text:"}
        ...
        <div className={styles.Buttons}>
          <LoadingButton isLoading={isLoadingUpdate} styleType="black">
            Update
          </LoadingButton>
        </div>
      </form>
